...
To achieve efficient \acrshort{spdc} processes, the energy and momentum must be conserved. A visualization of these processes is shown in \autoref{fig:SPDC}. An efficient \acrshort{spdc} process also means that the photon pair must interfere constructively and fulfill the phase-matching condition \cite{gilabertebassetPerspectivesApplicationsQuantum2019} : 
\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
\end{equation}
where the indices p, s and i refer to the pump, signal and idler photon. $\Delta \myvect{k}$ represents the phase mismatch caused by dispersion, which limits the efficiency of the photon pair generation. \newline
Most experiments use crystals to generate \acrshort{spdc} photons, such as \acrfull{ktp} and \acrfull{bbo}, or waveguides like \acrfull{ln}, because they exhibit second-order nonlinearity \cite{fiorentinoSpontaneousParametricDownconversion2007,kwiatUltrabrightSourcePolarizationentangled1999,tanzilliHighlyEfficientPhotonpair2000}.\newline
There are two approaches to compensate for the phase mismatch $\Delta \myvect{k}$ .
One is called \acrfull{qpm} and can be achieved in periodically poled crystals, e.g. \acrshort{ktp} or \acrshort{ln}, where the nonlinear response changes periodically, which compensates the mismatch. It allows for a process called type-0 \acrshort{spdc} to happen, which means that all three photons (pump, signal, idler) have the same polarization. \newline
...
\begin{figure}[!bt]
	...
	\resizebox{.7\textwidth}{!}{%
		\begin{tikzpicture}
			...
			\node [font=\LARGE, color={rgb,255:red,0; green,0; blue,255}] at (17.5,9.5) {$\myvect{k}_{\text{p}}$};
			\node [font=\LARGE, color={rgb,255:red,0; green,0; blue,0}] at (20.75,9.5) {$\Delta \myvect{k}$};
			\node [font=\LARGE, color={rgb,255:red,255; green,0; blue,0}] at (16.25,12) {$\myvect{k}_{\text{i}}$};
			\node [font=\LARGE, color=black!40!green] at (19.5,12) {$\myvect{k}_{\text{s}}$};
			
			\node [font=\LARGE] at (8,13.75) {\textbf{Energy\,\,conservation}};
			\node [font=\LARGE] at (18,13.75) {\textbf{Momentum\,\,conservation}};
	...
	\caption{Conservation processes of SPDC}
	\label{fig:SPDC}
\end{figure} \newline
In this work, the temporal correlation between the signal and idler photons is used for coincidence measurements. A tool for visualizing the coincidences is a coincidence histogram, which is shown in \autoref{fig:HistoExample}. It displays the number of coincident events as a function of the delay between the arrival times of the photons at the two single-photon detectors. \newline
...
\begin{figure}[b!]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% --- Histogram data ---
			\addplot+ [fill=black,fill opacity=0.7,draw=black,ybar interval,mark=no] plot coordinates {
				(-15, 48)
				(-13, 52)
				(-11, 46)
				(-9, 50)
...
\begin{equation}
	...
	\label{eq:SingleRef}
\end{equation}
where $N_{\text{tot}}^{\text{ref}}$ is the total number of counts without a sample, $N_{\mathrm{g}}$ is the number of generated photon pairs and $N_{\text{noise}}^{\text{ref}}$ is the number of counts when there are no photon pairs, which refer to noise consisting of e.g. dark counts of the detector or stray light. $\eta_{\text{idl}}$ corresponds to the efficiency of the idler arm, which includes the detection and coupling efficiency, as well as the arm's alignment. \newline
...
\begin{equation}
	...
	\label{eq:SingleSam}
\end{equation}
where $N_{\text{tot}}^{\text{sam}}$ represents the total counts in presence of a sample, $N_{\text{noise}}^{\text{sam}}$ the noise in presence of a sample and $T$ the transmittance of the sample. \newline
...
\end{equation}
As can be seen from the formula, four independent measurements must be conducted to retrieve the transmittance: two where only the noise is measured with and without the sample, and two with generated idler photons also with and without the sample. \newline
%\newpage
...
\begin{equation}
	...
	\label{eq:totCoinc}
\end{equation}
Using equations \ref{eq:pureCoinc} and \ref{eq:totCoinc}, the transmittance $T$ in the coincidence approach can be obtained as follows:
\begin{equation}
	T = 
...
Since the objective is to compare the precision of the transmittance $T$ using the two different approaches, analogue to \autoref{eq:VarianceTransGen}, the variance of the transmittance can be calculated using error propagation and coincidence counts:
\begin{equation}
	\begin{aligned}
		\operatorname{Var}(T) 
		&= \sum_{i} \left( \frac{\partial T}{\partial X_i} \right)^{2} 
...
The objective of this work is to ascertain parameter configurations in which the coincidence approach exhibits superiority in terms of precision, as measured by the magnitude of variance of the transmittance, in comparison to the conventional approach. To properly model both variance formulas, it is necessary to experimentally investigate the statistics of each parameter for which the variance needs to be determined in either \autoref{eq:VarianceTransExpl} or \autoref{eq:VarianceTransExplCoinc}.
...
The probability of detecting $n$ photons within $N$ subintervals of the total observation time $T$ corresponds to the case where $n$ subintervals each contain exactly one photon, while the remaining $(N-n)$ subintervals contain none. This event is described by the binomial distribution~\cite{foxQuantumOpticsIntroduction2006}:
\begin{equation}
...
\end{equation}
In the limit $N \to \infty$, it can be shown that the probability of detecting $n$ photons in a coherent light beam reduces to~\cite{foxQuantumOpticsIntroduction2006}:
\begin{equation}
