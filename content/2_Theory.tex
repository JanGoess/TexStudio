...
\begin{equation}
	...
	\end{aligned}
\end{equation}
where the indices p, s and i refer to the pump, signal and idler photon. $\Delta \myvect{k}$ represents the phase mismatch caused by dispersion, which limits the efficiency of the photon pair generation. A visualization of the conservation processes is shown in \autoref{fig:SPDC}. \newline
...
There are two approaches to compensate for the phase mismatch .
...
\begin{figure}[b!]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\node at (axis cs:-9, 300) [anchor=south] {\text{Accidentals}};
			
			\node at (axis cs:10, 300) [anchor=south] {\text{Accidentals}};
...
\end{equation}
As can be seen from the formula, four independent measurements must be conducted to retrieve the transmittance: two where only the dark counts are measured with and without the sample, and two with generated idler photons also with and without the sample. \newline
%\newpage
